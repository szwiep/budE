...
leresmodel = None
factor = None
whole_size_threshold = 700  # R_max from the paper
GPU_threshold = 1500 - 32 # Limit for the GPU (NVIDIA RTX 2080), can be adjusted 

# MAIN PART OF OUR METHOD
def run(dataset, option):

    # TODO: Simplify this for boosted-urban
    # We do not need midas/res/srlnet, etc. Just have IM2ELE in here for now
    # Add a step to estimate the resolution to use for all the images
    #   - This will be justified in video/report as a way to speed up the process
    #   by taking advantage of the lower-quality images + geometric consistency +
    #   types of high frequency details in aerial/satellite images
    ...
    for image_ind, images in enumerate(dataset):
        ...
        if option.R0 or option.R20:
            if option.output_resolution == 1:
                midas.utils.write_depth(path, cv2.resize(whole_estimate, (440, 440),
...
def doubleestimate(img, size1, size2, pix2pixsize, net_type, path):
    ...
    midas.utils.write_depth(path + 'low_est', cv2.resize(estimate1, (440, 440),
                                                        interpolation=cv2.INTER_CUBIC),
...
# TODO: try not going to PIL first, stay in numpy as go straight to tensor from here
def estimateim2ele(img, msize):

    # WITH PIL
    # Crop to deal with cropped training training environemnt (source borders)
    ...
    # im2ele needs iamge to be in PIL format
    ...
    im2ele_transform = transforms.Compose([
                                                # Scale(msize, interpolation=Image.BICUBIC),
                                                # Image.resize(msize, resample=Image.BICUBIC, box=None, reducing_gap=None),
                                                # ,
                                                ToTensor(),
                                                Normalize([0.485, 0.456, 0.406],
                                                            [0.229, 0.224, 0.225])
                                            ])
    # img_torch = im2ele_transform(A_resize.astype(np.float32))
    ...
    # TODO: try this cv2.resize vs. torch.nn.functional
    # prediction = torch.nn.functional.interpolate(prediction, size=(440, 440), mode='bilinear')   
    prediction = prediction.squeeze().cpu().numpy()
...
if __name__ == "__main__":
    ...
    if option_.depthNet == 0:
        ...
        option_.net_receptive_field_size = 440
        option_.patch_netsize = 2 * option_.net_receptive_field_size
    else:
        assert False, 'depthNet can only be 0, 1, 2, or 3'
